...
| 3/15 | Q3-Diffusion（30%） | forward Markov链；T-step闭式；reverse/latent diffusion；Generative Trilemma | EP: 69–82, 100；Diffusion: 19–45, 53–60, 71–78（latent diffusion）（选看 Video: 55–63） | 要求：forward transition 与 forward jump（两步到一般步）推导主线；latent diffusion 和 trilemma 结论；不要求：论文系统实现细节。 | 计时推导/写结论：`q(x_t given x_0)` 等关键式重复练 |
