...
| 3/10 | Q2-Transformer&Attn（55%）(1/2) | scaled dot-product attention；BERT vs. GPT objectives | EP: 29–33, 38–39；Text: 3–6, 33–35, 41–43 | 要求：按 EP 第3页先抓前半考点，重点是 scaled dot-product attention（含 attention scaling）和 BERT vs. GPT objectives；不要求：把 tokenization、KV cache、teacher forcing 当成 Q2 重点单独展开；autoregressive 只需掌握到 GPT objective 这一层。 | 整理 1 页速记：attention 公式、attention scaling 含义、BERT vs. GPT objectives 对照；计时短题 4–6 题（每题 1–3 分钟） |
| 3/11 | Q2-Transformer&Attn（55%）(2/2) + LoRA（10%） | complexity；Vision Transformers (ViT)；LoRA | EP: 3, 46；Text: 40–44, 75–81；Video: 41–42（ViT 相关） | 要求：按 EP 第3页补 transformer 后半考点，重点是 complexity、ViT；LoRA 重点看 rank、parameter number/efficiency、initialization；不要求：扩展到 Flash Attention、MoE、RLHF 细节。 | 整理 1 页速记：complexity、ViT、LoRA（rank / parameter efficiency / initialization）；计时短题 4–6 题（每题 1–3 分钟） |
